# Function to add a mini-map of anomalies to a time-series plot
import os
import datetime
import numpy
import math
...
def miniMap(
    ...
    # Make the climatology - slow, so cache it
    clim = None
    climfile = "%s/cache/Yangztze_climatologies/%s/%04d%02d%02d%02d.nc" % (
        os.getenv("SCRATCH"),
        var,
        year,
        month,
    ...
    if os.path.exists(climfile):
        clim = iris.load_cube(climfile)
    else:
        count = 0
        for offset in [-4, -3, -2, -1, 1, 2, 3, 4]:
            dto = datetime.datetime(dte.year + offset, dte.month, dte.day, dte.hour)
            f = loadSmoothed(var, dto, 15 * 24, version)
            if clim is None:
                clim = f.copy()
            else:
                clim.data += f.data
            count += 1
        clim /= count
        if not os.path.isdir(os.path.dirname(climfile)):
            os.makedirs(os.path.dirname(climfile))
        iris.save(clim, climfile)
    ...
    if var == "PRMSL":
        ...
        for i in numpy.arange(cmap.N):
            idf = abs(i - cmap.N / 2)
            stcmap[i, 3] = min(0.1, 1 / (1 + math.exp(-0.25 * (idf - cmap.N / 10))))
    ...
    if var == "TMP2m":
        ...
        for i in numpy.arange(cmap.N):
            idf = abs(i - cmap.N / 2)
            stcmap[i, 3] = 1 / (1 + math.exp(-0.25 * (idf - cmap.N / 10)))
        stcmap = matplotlib.colors.ListedColormap(stcmap)
        pImg = ax.pcolormesh(
            lons,
            lats,
    ...
    if var == "PWAT":
        ...
        for i in numpy.arange(cmap.N):
            idf = abs(i - cmap.N / 2)
            stcmap[i, 3] = 0.1 * 1 / (1 + math.exp(-0.25 * (idf - cmap.N / 8)))
        stcmap = matplotlib.colors.ListedColormap(stcmap)
        pImg = ax.pcolormesh(
            lons,
            lats,
    ...
    if var == "WEASD":
        ...
        for i in numpy.arange(cmap.N):
            idf = abs(i - cmap.N / 2)
            stcmap[i, 3] = 1 / (1 + math.exp(-0.25 * (idf - cmap.N / 8)))
        stcmap = matplotlib.colors.ListedColormap(stcmap)
        pImg = ax.pcolormesh(
            lons,
            lats,
