...
# 20CRv3 anomaly time-series: 3-hourly data, catchment average.
...
from miniAnomalyMap import miniMap
...
for dec in [-4, -3, -2, -1, 1, 2, 3, 4]:
    ...
    (ndd, dtsd) = fromversion(args.version, year_offset=dec, pad_days=35)
    ndda.append(ndd)
    if ndd_a is None:
        ndd_a = ndd.copy()
    else:
...
for m in range(80):
    (dtsrm, rmem) = movingaverage(dts, ndata[:, m] * args.yscale, 3 * 8)
    ax.add_line(
        Line2D(
            dtsrm,
            rmem,
...
for idx in range(len(ndda)):
    # Add the running mean of the ensemble mean for the comparison year
    ndde = ensm(ndda[idx])
...
fig.savefig("%s_anomaly_story.png" % args.var)
