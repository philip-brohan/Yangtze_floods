...
# Plot v3, v4.6.1 and the results at the new stations
...
opdir="%s/images/Yangtze_1931_v3_v_4.6.1" % os.getenv('SCRATCH')
...
# In the centre - spaghetti-contour plot of v4.6.1
...
prmsl_s=twcr.load('prmsl',dte,version='4.6.1')
obs_t_s=twcr.load_observations_fortime(dte,version='4.6.1')
...
for y in range(0,len(stations)):
    ...
    ax_right.add_line(matplotlib.lines.Line2D(
            xdata=(obsn_d,obsn_d), ydata=(y+1.1,y+1.9),
            linestyle='solid',
            linewidth=3,
            color=(0,0,0,alpha),
            zorder=1))
...
# Output as png
fig.savefig('%s/%04d%02d%02d%02d%02d.png' % 
            (opdir,args.year,args.month,args.day,
             int(args.hour),int(args.hour%1*60)))
