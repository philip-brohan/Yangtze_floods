...
opdir="%s/20CR/version_3/analyses/Yangtze_catchment_ts/TMP2m" % os.getenv('SCRATCH')
...
for year in range(1926,1937):
    for month in range(1,13):
        ...
        for member in range(len(mtmp)):
            tSeries[member].append((mtmp[member])-273.15)
...
fig.savefig('TMP2m_ts_actuals.png')
