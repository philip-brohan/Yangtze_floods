...
# Compare pre-calculated stripes from 20CRv3 and scout 4.6.7
# Add axes and a grid.
...
# Assemble the 4.6.7 sample
dts=[]
ndata_scout=None
current=start
while current<=end:
    try:
        sfile=("%s/20CR/version_4.6.7/analyses/Stripes_daily/TMP2m/"+
...
while current<=end:
    try:
        sfile=("%s/20CR/version_3/analyses/Stripes_daily/467-TMP2m/"+
             "%04d%02d%02d.pkl") % (os.getenv('SCRATCH'),
                                    current.year,current.month,current.day)
        with open(sfile, "rb") as f:
           nddy  = pickle.load(f)
